...
<div class="admin-page-header">
    <div>
        ...
        <h2 class="text-2xl font-black mt-0.5">Add Category</h2>
    </div>
</div>

<form method="POST" enctype="multipart/form-data" action="{{ route('admin.categories.store') }}" class="bg-white border border-slate-200 rounded-xl p-6 shadow-sm">
    @csrf
    <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
        <div>
            <label class="block font-bold text-sm mb-1">Category Name</label>
            <input class="admin-input @error('name') is-invalid @enderror" name="name" value="{{ old('name') }}" placeholder="e.g. Wedding" required>
        ...
        <div class="md:col-span-2">
            ...
            <textarea class="admin-input @error('description') is-invalid @enderror" name="description" placeholder="Brief description of this category" rows="3" style="min-height:80px;">{{ old('description') }}</textarea>
            @error('description')<div class="text-red-600 text-xs mt-1">{{ $message }}</div>@enderror
        </div>
    </div>
    <div class="flex gap-2 mt-4">
        <button class="btn-admin"><i class="fas fa-floppy-disk"></i> Save Category</button>
