...
<div class="admin-page-header">
    <div>
        ...
        <h2 class="text-2xl font-black mt-0.5">Add Package</h2>
    </div>
</div>

<form method="POST" action="{{ route('admin.packages.store') }}" class="bg-white border border-slate-200 rounded-xl p-6 shadow-sm">
    @csrf
    <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
        <div>
            <label class="block font-bold text-sm mb-1">Package Name</label>
            <input class="admin-input @error('name') is-invalid @enderror" name="name" value="{{ old('name') }}" placeholder="e.g. Silver Wedding" required>
        ...
        <div>
            ...
            <div class="flex">
                ...
                <input class="admin-input rounded-l-none @error('price') is-invalid @enderror" name="price" value="{{ old('price') }}" type="number" step="0.01" placeholder="49999" required>
            </div>
        ...
        <div class="md:col-span-2">
            ...
            <textarea class="admin-input @error('features') is-invalid @enderror" name="features" placeholder="List key features (one per line)" rows="4" style="min-height:100px;">{{ old('features') }}</textarea>
            @error('features')<div class="text-red-600 text-xs mt-1">{{ $message }}</div>@enderror
        </div>
    </div>
    <div class="flex gap-2 mt-4">
        <button class="btn-admin"><i class="fas fa-floppy-disk"></i> Save Package</button>
